    ...
    for _,_,filenames in tqdm(walk(path0)):
        for filename in filenames:
            ...
            if os.path.exists(join(path0, file_prefix + ".tif")):
                ...
                print("wrong file types or no such files")
    return picture
...
def get_patch(data_set, random_set, batch, widths, heights):
    random_choice = np.random.choice(data_set)

    [[effective_x,effective_y,effective_widths,effective_heights],
     [tumor_leftup_x,tumor_leftup_y,mask_effective_widths,mask_effective_heights]] = random_set[random_choice]
...
#                     print("取tumor随机点坐标是：%d,%d"%(random_x,random_y))
        ...
        random_img01 = (random_img.convert("RGB")).rotate(np.random.random(1) * 180.)

        #***接下来就给他贴标签，并处理成训练所需的数据结构***#
        random_img_arr = np.array(random_img01).astype(np.float32)    # zero-center 需将数据类型变换为float型
        # zero-center 处理，r-mean(r) 然后/std(r) 
        random_img_arr[:,:,0] = (random_img_arr[:,:,0] - (random_img_arr[:,:,0]).mean())/(random_img_arr[:,:,0].std())
        random_img_arr[:,:,1] = (random_img_arr[:,:,1] - (random_img_arr[:,:,1]).mean())/(random_img_arr[:,:,1].std())
        random_img_arr[:,:,2] = (random_img_arr[:,:,2] - (random_img_arr[:,:,2]).mean())/(random_img_arr[:,:,2].std())
        return random_img_arr, 0    # 注意要除以255.,做归一化处理
...
#                 print("随机情况",len(random_img_x),(array(random_img.convert("RGB"))).std())
        while (array(random_img.convert("RGB"))).std()<10.0 or len(random_img_x) != 0:
...
#                 print("非tumor区检测情况",len(random_img_x), (array(random_img.convert("RGB"))).std())
        ...
        #***接下来就给他贴标签，并处理成训练所需的数据结构***#
        random_img_arr = array(random_img01).astype(np.float32)
        random_img_arr[:,:,0] = (random_img_arr[:,:,0] - (random_img_arr[:,:,0]).mean())/(random_img_arr[:,:,0].std())
        random_img_arr[:,:,1] = (random_img_arr[:,:,1] - (random_img_arr[:,:,1]).mean())/(random_img_arr[:,:,1].std())
        random_img_arr[:,:,2] = (random_img_arr[:,:,2] - (random_img_arr[:,:,2]).mean())/(random_img_arr[:,:,2].std())
        return random_img_arr, 1    # 注意要除以255.,做归一化处理
...
def data_generator(data_set, batch=32, widths=256, heights=256):
    ...
    while True:
        with ThreadPoolExecutor(max_workers=32) as executor:
            patch_threader = executor.submit(get_patch, data_set, random_set, batch, widths, heights)
